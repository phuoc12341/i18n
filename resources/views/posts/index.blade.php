                    ...
                    <tr>
                        ...
                        <td>
                            @if ( $post->postTranslations->isNotEmpty() )
                                <a class="btn btn-warning" href="{{ route('postTranslations.edit', ['postTranslation' => $post->translation_id]) }}"><i class="fas fa-pen-square"></i></a>

                                <!-- Button trigger modal -->
                                <button type="button" class="btn btn-danger deleteButton" data-post-title="{{ $post->title }}" data-post-id="{{ $post->id }}" data-toggle="modal" data-target="#deleteModal"><i class="fas fa-trash-alt"></i></button>
                            @else
                                <a class="btn btn-success" href={{ route('posts.create', ['postId' => $post->id]) }}><i class="fas fa-plus"></i></a>
                            @endif
                        </td>
                    </tr>
                @endforeach
...
@section('script')
    <script>
        ...
        $('.deleteButton').click(function() {
            let formTitle = $(this).data('post-title')
            let deleteForm = $('#deleteForm')
            let postId = $(this).data('post-id')
            deleteForm.attr('action', baseActionDelete + '/' + postId)
            let modalData = $("#modalData")
            modalData.text(formTitle)
        });
    </script>
@endsection
